...
const Ada = function(gitlabUrl, projectId, token){
...
Ada.prototype.getPipelineId = async function(commitId){
    ...
    const url = `${this._gitlabUrl}/api/v3/projects/${projectId}/repository/commits/${commitId}?private_token=${token}`;
...
Ada.prototype.getJobId = async function(pipelineId){
    const token = this._token;
    const projectId = this._projectId;
    const url = `${this._gitlabUrl}/api/v3/projects/${projectId}/pipelines/${pipelineId}/jobs?private_token=${token}`;
    const response = await axios.get(url);
    if(response.status !== 200){
        throw new Error('request error with status code:' + response.status);
    }
...
Ada.prototype.downloadArtifacts = async function(jobId, path){
    const token = this._token;
    const projectId = this._projectId;
    const url = `${this._gitlabUrl}/api/v3/projects/${projectId}/jobs/${jobId}/artifacts?private_token=${token}`;
    const response = await axios.get(url, {
        responseType: 'stream'
    });
